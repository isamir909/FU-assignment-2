...
const regionModel=require("../../model/admin/property/regionModel")
const propertyModel=require("../../model/admin/property/propertyModel")
const fieldModel=require("../../model/admin/property/fieldModel")
const cropeCycle=require("../../model/admin/crop/cropCycleFieldModel")
...
const createCropCycleField=async (req,res)=>{
  try {
      let data=req.body
      const createData=await cropeCycle.create(data)
      return res.status(201).send({status:true,message:"created successfully",data:createData})
  } catch (error) {
    console.log(error);
    return res.status(500).send({ status: false, msg: error.message })
